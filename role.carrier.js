...
var roleCarrier = {
  ...
  run: function (creep) {
    ...
    if (creep.memory.task == 'reap') {
      ...
      if (spawners.length > 0) {
        ...
        //   Memory.Foo = containers[0].store.energy;
        //   if (containers.length > 0 && containers[0].store.energy > 1000) {
        ...
        //       creep.moveTo(storage[0].pos, { visualizePathStyle: { stroke: '#ffffff' } });
        //     }
        //   }
        // }
      }
  ...
  build: function (creepMem) {
    ...
    return Game.spawns[creepMem.memory.spawn].spawnCreep(this.body, newName, { memory: { role: 'Carrier', task: 'reap' } });
  }
};
