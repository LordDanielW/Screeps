  ...
  run: function (creep) {
    ...
    switch(creep.memory.task){
      case "getTask":
        ...
        if(creep.store.getUsedCapacity() != 0 ){creep.memory.task = "Empty";}
        else if(creep.room.terminal.store[RESOURCE_ENERGY] < 5000){creep.memory.task = "getEnergy"}
        //else if(factory.store[RESOURCE_ENERGY] < 1000) {creep.memory.task = "getBat";}
        else if(factory.store[RESOURCE_LEMERGIUM_BAR] > 200) {creep.memory.task = "getLegBar";} //_BAR
        else if(factory.store[RESOURCE_LEMERGIUM] < 1000) {creep.memory.task = "getLeg";}
        //else if(factory.store.lem) 
        //else{creep.memory.task = "getEnergy";}
        break;
        
      case "Empty":
         if(roleUtilities.emptyCarry(creep)){ creep.memory.task = "getTask";}
        break;
        
      case "getBat":
        if(creep.withdraw(creep.room.storage, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE)
        { creep.moveTo(creep.room.storage.pos, {visualizePathStyle:{stroke: '#00ffff'}});} 
        else{creep.memory.task = "giveBat";}
        break;
      case "giveBat" :
        var factory = creep.room.find(FIND_STRUCTURES, {filter: (d) =>{return(d.structureType == STRUCTURE_FACTORY);}})[0];
        ...
        else{ creep.memory.task = "getTask";}
      break;
        
      case "getLeg":
        if(creep.withdraw(creep.room.terminal, RESOURCE_LEMERGIUM) == ERR_NOT_IN_RANGE)
        { creep.moveTo(creep.room.terminal.pos, {visualizePathStyle:{stroke: '#00ffff'}});} 
        else{creep.memory.task = "giveLeg";}
        break;
      case "giveLeg" :
        var factory = creep.room.find(FIND_STRUCTURES, {filter: (d) =>{return(d.structureType == STRUCTURE_FACTORY);}})[0];
        //Memory.Foo = (factory.store.battery < 5000);
      ...
      case "getLegBar":
        var factory = creep.room.find(FIND_STRUCTURES, {filter: (d) =>{return(d.structureType == STRUCTURE_FACTORY);}})[0];
        if(creep.withdraw(factory, RESOURCE_LEMERGIUM_BAR) == ERR_NOT_IN_RANGE)
        {creep.moveTo(factory);} 
        else{ creep.memory.task = "giveLegBar"; }
      break;
      case "giveLegBar":
        if(creep.transfer(creep.room.terminal, RESOURCE_LEMERGIUM_BAR) == ERR_NOT_IN_RANGE)//_BAR
      ...
      case "getEnergy":
        ...
        //  break;
      case "giveEnergyLink":
        var DestID = creep.pos.findClosestByRange(FIND_STRUCTURES, { filter: (structure) => { return (structure.structureType == STRUCTURE_LINK);}});
      ...
      case "giveEnergyTerminal":
        if(creep.transfer(creep.room.terminal, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE)
          { creep.moveTo(creep.room.terminal, {visualizePathStyle:{stroke: '#00ffff'}});}
        else{ creep.memory.task = "getTask"; }
        break;
      default:
